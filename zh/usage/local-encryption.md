...
普通安装版会把解密所需的本地数据密钥交给操作系统的安全存储管理；Windows 便携版不会使用系统钥匙串保存本地数据密钥，而是把被口令加密后的密钥保存到便携数据目录中的 `portable-key.json`。

这意味着：数据可以在你的设备上正常使用，但我们无法拿到你的本地数据密钥，也无法在没有密钥或同步密码的情况下帮你解开数据。

## 本地数据如何加密

首次启动时，Termark 会生成一个 32 字节随机密钥，作为本地数据加密密钥。这个密钥不会写入 Termark 的普通数据库文件。
...
Windows 便携版使用独立的 portable 密钥模式：Termark 会要求你设置一个本地加密口令，并使用该口令派生出的密钥加密本地数据密钥，然后把加密后的结果保存到程序目录下的 `data/backend/portable-key.json`。这个文件不会保存你的明文口令，也不会保存明文的本地数据密钥；文件内容包含 KDF 参数、salt、nonce 和加密后的本地数据密钥。

便携版每次启动时需要先用本地加密口令解锁 `portable-key.json`，解出本地数据密钥后，才能继续读取和解密数据库中的敏感字段。如果 `portable-key.json` 丢失、损坏，或者你忘记了本地加密口令，原有数据库里的密码、私钥等敏感字段将无法解密。

数据库写入时，Termark 会使用这个本地数据密钥，通过 AES-256-GCM 加密敏感字段。每次加密都会生成新的随机 nonce，所以即使两次保存相同内容，落盘后的密文通常也不同。

数据库读取时，Termark 会先读取本地数据密钥，再解密数据库中的敏感字段，然后提供给应用界面和连接逻辑使用。普通安装版会从系统 keychain 读取 `local-data-key`；Windows 便携版会通过你输入的本地加密口令解锁 `portable-key.json`。如果对应的密钥来源不可用，或者本地数据密钥无法读取，Termark 会拒绝以未加密状态继续写入新数据。
...
- Windows 便携版的本地数据密钥保存在 `data/backend/portable-key.json` 中，但它会先被你的本地加密口令加密，不会以明文形式落盘。
- 同步数据需要你设置的同步密码才能解密。
- 同步密码不会上传给服务端；即使选择记住，也只是保存在你设备的系统 keychain 中。
- AES-GCM 带有完整性校验，使用错误密钥或错误同步密码时，解密会失败，而不是得到可读数据。

因此，如果你只提供数据库文件、同步密文或服务端数据，我们无法从中还原你的密码、私钥或其他敏感字段。没有对应的系统 keychain 条目、Windows 便携版的 `portable-key.json` 与本地加密口令，或同步密码，密文对我们也是不可读的。
...
- 使用 Windows 便携版时，只复制了数据库文件，没有同时复制 `data/backend/portable-key.json`。
- 使用 Windows 便携版时，忘记了首次设置的本地加密口令，或 `portable-key.json` 被删除、损坏。
...
- 迁移 Windows 便携版数据时，请完整复制程序目录下的 `data` 目录，尤其是 `data/backend/portable-key.json`，并妥善保存你设置的本地加密口令。
- 启用同步时，请妥善保存同步密码，例如放入你信任的密码管理器。
- 在旧设备仍可使用时完成新设备登录和同步确认，再清理旧设备。
- 如果你需要长期归档，请同时确认本地数据、对应密钥来源和同步密码的可用性。普通安装版需要确认系统 keychain 可用；Windows 便携版需要确认 `portable-key.json` 和本地加密口令可用。
